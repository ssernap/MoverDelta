function varargout = moverGUI2_1(varargin)
% MOVERGUI2_1 MATLAB code for moverGUI2_1.fig
%      MOVERGUI2_1, by itself, creates a new MOVERGUI2_1 or raises the existing
%      singleton*.
%
%      H = MOVERGUI2_1 returns the handle to a new MOVERGUI2_1 or the handle to
%      the existing singleton*.
%
%      MOVERGUI2_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MOVERGUI2_1.M with the given input arguments.
%
%      MOVERGUI2_1('Property','Value',...) creates a new MOVERGUI2_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before moverGUI2_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to moverGUI2_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help moverGUI2_1

% Last Modified by GUIDE v2.5 11-Sep-2016 19:09:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @moverGUI2_1_OpeningFcn, ...
                   'gui_OutputFcn',  @moverGUI2_1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before moverGUI2_1 is made visible.
function moverGUI2_1_OpeningFcn(hObject, eventdata, handles, varargin)
global coordenadas ingresadas HomeTH1 HomeTH2 HomeTH3
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to moverGUI2_1 (see VARARGIN)

% Choose default command line output for moverGUI2_1
...
% UIWAIT makes moverGUI2_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = moverGUI2_1_OutputFcn(hObject, eventdata, handles) 
...
DesNorm=(TiempoAcu(i+1)-TiempoAcu(i));
 ...
 irlineal(PosDeseadaTH1,PosDeseadaTH2); 
...
grid on
    ...
    function val=deg2motor(ThetaM,HomeTH)
        val=Deg2Digital((ThetaM)-HomeTH-90);
        
        
    function val=rad2motor(ThetaM,HomeTH)
        val=Deg2Digital(rad2deg(ThetaM)-HomeTH-90);
...
global HomeTH1 HomeTH2 HomeTH3
    ...
    function irlineal(PosDeseadaTH1,PosDeseadaTH2)      
   ...
   % t=0;
    ...
    %    t=t+1;
     ...
     SyncWrite([ath1*Time+bth1,ath2*Time+bth2]);
     while(toc-Time<0.0012)

     end
     Time = toc;
    end
    SyncWrite([PosDeseadaTH1,PosDeseadaTH2]);
    pause(1);
