...
for i = 1:1:length(Th)-1 
   ...
   if(i==length(Th)-1 )
   R=[Th(i);Velocidades(i)/T(i-1);0;Th(i+1);0;0]; %vector respuesta FINAL
   else
       if(i==1)
           R=[Th(i);0;0;Th(i+1);Velocidades(i+1)/T(i+1);0]; %vector respuesta INICIAL
       else
           R=[Th(i);Velocidades(i)/T(i-1);0;Th(i+1);Velocidades(i+1)/T(i+1);0]; %vector respuesta
       end
   end
